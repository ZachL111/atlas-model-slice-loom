...
`atlas-model-slice-loom` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for slice workflows, centered on protocol validation, framed sample traffic, and bounds and ordering tests.
...
The score is deterministic. The current threshold is 164. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 5.
